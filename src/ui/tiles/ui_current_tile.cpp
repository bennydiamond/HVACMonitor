#include "ui/tiles/ui_current_tile.h"
#include "ui/custom_icons.h"

const int SCROLL_SPEED_MS_PX_SEC = 5;

UICurrentTile::UICurrentTile(ConfigManager* config) : _config(config) {}

lv_obj_t* UICurrentTile::create_tile(lv_obj_t* parent_tv) {
    ...
    lv_label_set_text(compressor_icon, ICON_ENGINE); // Proposed icon for compressor
    ...
    lv_label_set_text(pump_icon, ICON_WATER_PUMP); // Proposed icon for pump
    ...
    lv_label_set_text(pump_label, "Pump: --- A");
...
void UICurrentTile::set_fan_status_icon(lv_obj_t* icon) {
    fan_alert_icons.status_bar_icon = icon;
}

void UICurrentTile::clear_readings() {
    lv_label_set_text(fan_label, "Fan: --- A");
    lv_label_set_text(compressor_label, "Comp: --- A");
    lv_label_set_text(pump_label, "Pump: --- A");
...
void UICurrentTile::update_fan_current(float amps, FanStatus status) {
...
void UICurrentTile::update_compressor_current(float amps) {
...
void UICurrentTile::update_pump_current(float amps) {
    lv_label_set_text_fmt(pump_label, "Pump: %.2f A", amps);
...
void UICurrentTile::update_fan_status(FanStatus status) {
...
void UICurrentTile::fan_alert_blink_cb(lv_timer_t* timer) {
